          ...
          <el-tab-pane label="商品参数" name="1">
            ...
            <el-form-item v-for="item in manyTableData" :label="item.attr_name" :key="item.attr_id">
              ...
              <el-checkbox-group v-model="item.attr_vals">
                <el-checkbox v-for="(cb, i) in item.attr_vals" :key="i" border :label="cb"></el-checkbox>
              </el-checkbox-group>
            </el-form-item>
          </el-tab-pane>
...
export default {
  data () {
    return {
      // 控制步骤条显示步骤，步骤条active(数字类型)和tabs的v-model共用，实现同步
      activeIndex: '0', // tabs的v-model绑定值是选项的name，(字符串类型)，
      // 表单数据对象
      addForm: {
        goods_name: '',
        goods_price: 0,
